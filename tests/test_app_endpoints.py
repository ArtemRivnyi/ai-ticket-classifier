...
def test_status_endpoint(client, headers, mocker):
    ...
    if classifier:
        mocker.patch.object(
            classifier,
            "get_status",
            return_value={"gemini": "available", "openai": "unavailable"},
        )
...
def test_classify_endpoint_all_categories(client, mocker, headers):
    ...
    for category in categories:
        ...
        response = client.post(
            "/api/v1/classify", json={"ticket": f"Test {category}"}, headers=headers
        )
        assert response.status_code == 200
        data = response.get_json()
...
def test_batch_endpoint_empty_list(client, headers):
    ...
    response = client.post("/api/v1/batch", json={"tickets": []}, headers=headers)
    assert response.status_code == 400
...
def test_error_handler_400(client, headers):
    ...
    response = client.post(
        "/api/v1/classify", json={"invalid": "data"}, headers=headers
    )
    assert response.status_code == 400
...
def test_error_handler_500(client, mocker, headers):
    ...
    mocker.patch(
        "middleware.auth.APIKeyManager.get_key_data",
        return_value={
            "id": "test_key_id",
            "key_hash": "test_hash",
            "user_id": "test_user",
    ...
    if classifier:
        mocker.patch.object(
            classifier, "classify", side_effect=Exception("Internal error")
        )
        mocker.patch.object(classifier, "gemini_available", True)
    else:
        mock_classifier = mocker.Mock()
    ...
    response = client.post("/api/v1/classify", json={"ticket": "Test"}, headers=headers)
...
def test_batch_service_unavailable(client, mocker, headers):
    ...
    response = client.post("/api/v1/batch", json={"tickets": ["Test"]}, headers=headers)
    assert response.status_code == 503
