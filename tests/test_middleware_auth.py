...
from middleware.auth import APIKeyManager, RateLimiter, require_api_key, optional_api_key, TIER_LIMITS
...
def test_api_key_manager_get_key_data(mock_redis_client):
    ...
    assert key_data.get('id') == 'key_id' or 'test_key_id' in str(key_data.get('id', '')) or key_data.get('id') is not None
