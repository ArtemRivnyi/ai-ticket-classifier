...
for f in files:
    ...
    content = content.replace(
        ...
        'mock_classifier = mocker.MagicMock(); app.config["CLASSIFIER"] = mock_classifier',
    )

    # Add `app` to the function arguments
    content = re.sub(
        r"def (test_[a-zA-Z0-9_]+)\((.*?mocker.*?)\):",
        lambda m: "def "
        + m.group(1)
        + "("
        + (
            m.group(2)
            if "app" in [arg.strip() for arg in m.group(2).split(",")]
            else m.group(2).replace("mocker", "app, mocker")
        )
        + "):",
        content,
    )
