...
def get_analytics_stats():
    ...
    try:
        ...
        accuracy_data = [
            {
                "date": str(d.date),
                "accuracy": round(float(d.avg_accuracy or 0) * 100, 2),
            }
        ...
        categories = (
            db.session.query(Feedback.predicted, func.count(Feedback.id).label("count"))
        ...
        volume_data = [{"date": str(v.date), "count": v.count} for v in volume]
